...
//	float r[10] = {0};
	...
	if (n >= 0){
		for (cnt = 2; cnt < n; ++cnt){
			...
			fscanf (fp, "rms = %f", &rms[cnt-2]);
			/* ^just edited, check */
			printf ("%f\n", rms[cnt-2]);


			exp_fitness[cnt-2] =/* exp (- abs*/ (rms[cnt-2] - 0)/* ) */;
			printf ("%f\n", exp_fitness[cnt-2]);
	...
	qsort (threshold, 10, sizeof(float), compare_floats);
	int i;
	for (i = 0; i < 10; i++){
		if (/* r[i] !=0 && */ exp_fitness[i] > threshold[8]){
	...
	/* START of RECOMBINATION */
		...
		char command[50];
                strcpy (command, "export RAND_MAX=$RANDOM");
                system (command);
